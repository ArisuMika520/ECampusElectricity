#!/bin/bash
# 分支分离脚本：将 mode1 和 mode2 分离为两个分支
# mode1 -> Web 分支
...
if ! git diff-index --quiet HEAD -- || [ -n "$(git ls-files --others --exclude-standard)" ]; then
    echo -e "${YELLOW}检测到未提交的更改或未跟踪的文件${NC}"
    echo -e "${CYAN}当前状态：${NC}"
    git status --short
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        if [ -z "$commit_msg" ]; then
            commit_msg="chore: 分离 mode1 和 mode2 为不同分支"
        fi
        git add -A
        git commit -m "$commit_msg"
        echo -e "${GREEN}✓ 更改已提交${NC}"
    else
        echo -e "${RED}请先提交或暂存所有更改后再运行此脚本${NC}"
        exit 1
    fi
fi
...
if git show-ref --verify --quiet refs/heads/Web; then
    echo -e "${YELLOW}Web 分支已存在${NC}"
    read -p "是否删除并重新创建 Web 分支？(y/n) " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        git branch -D Web
        git checkout -b Web
...
# 3. 在 Web 分支上，确保根目录文件存在（如果需要，可以从 main 保留）
