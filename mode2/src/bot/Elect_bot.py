...
class EnhancedQQBot(botpy.Client):
    ...
    async def on_group_at_message_create(self, message: GroupMessage):
        ...
        # 电费图形 - 历史
        elif content.startswith("/图形化历史") or content.startswith("/图形化-历史"):
            plot_params = bot_command.Content_split.plot_history(content)
            if plot_params:
                await self._send_plot(message, "历史", plot_params["room_name"], plot_params["time_span"])
            else:
                await self.send_group_help(message)

        # 电费图形 - 消耗
        elif content.startswith("/图形化消耗") or content.startswith("/图形化-消耗"):
            plot_params = bot_command.Content_split.plot_consumption(content)
            if plot_params:
                await self._send_plot(message, "消耗", plot_params["room_name"], plot_params["time_span"])
            else:
                await self.send_group_help(message)
    ...
    async def _send_plot(self, message: GroupMessage, plot_type: str, room_name: str, time_span: int):
